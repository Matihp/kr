...
import { Clock, DollarSign, AlertTriangle, Check, Award, Layers } from 'lucide-react'
...
export default function GigDetailPage() {
  ...
  useEffect(() => {
    if (!loading && user && user.userType !== 'recruiter') {
      router.push('/gigs')
    }
  }, [loading, user, router])
  ...
  const getStatusColor = (status: string | undefined) => {
    // Si status es undefined, asigno un valor por defecto
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="border-none shadow-md hover:shadow-lg transition-shadow duration-300">
            ...
            <CardContent className="pt-6">
              <p className="whitespace-pre-wrap text-gray-700 leading-relaxed">{gig.description}</p>
            </CardContent>
          </Card>
              ...
              <CardContent className="pt-6">
                <div className="space-y-4">
                  {gig.stages.map((stage, index) => (
                    <div key={stage.id} className="border border-gray-200 rounded-xl p-5 hover:border-purple-300 transition-colors duration-300">
                      <div className="flex justify-between items-center">
                        ...
                        <Badge className={`${stage.isCompleted ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'} px-3 py-1 rounded-full flex items-center gap-1`}>
                          {stage.isCompleted ? <Check size={14} /> : <Clock size={14} />}
                          {stage.isCompleted ? "Completado" : "Pendiente"}
                        </Badge>
                      </div>
                      <p className="text-gray-600 mt-3 leading-relaxed">{stage.description}</p>
                      <div className="flex justify-between mt-4 text-sm bg-gray-50 p-3 rounded-lg">
                        ...
                        <span className="font-bold text-green-600">${stage.payment.toLocaleString()}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <CardContent className="pt-6">
                <div className="space-y-4">
                  {gig.rewards.map((reward) => (
                    <div key={reward.id} className="border border-gray-200 rounded-xl p-5 hover:border-amber-300 transition-colors duration-300">
                      <h3 className="font-medium text-lg flex items-center gap-2">
                        <Award size={18} className="text-amber-500" />
                        {reward.name}
                      </h3>
                      <p className="text-gray-600 mt-3 leading-relaxed">{reward.description}</p>
                      <div className="flex justify-between mt-4 text-sm bg-amber-50 p-3 rounded-lg">
                        <span className="font-medium text-gray-500 flex items-center gap-1">
                          <DollarSign size={16} className="text-amber-500" />
                          Monto del bono:
                        </span>
                        <span className="font-bold text-amber-600">${reward.amount.toLocaleString()}</span>
                      </div>
                      <div className="mt-4">
        ...
        <div className="space-y-8">
            ...
            <CardContent className="pt-6">
              <div className="bg-white p-5 rounded-xl border border-gray-100">
                <div className="flex justify-between items-center text-lg pb-4 border-b">
                  ...
                  <span className="font-bold text-green-600">${gig.budgetMin.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center text-lg pt-4">
                  <span className="font-medium text-gray-600">Máximo:</span>
                  <span className="font-bold text-green-600">${gig.budgetMax.toLocaleString()}</span>
                </div>
              </div>             
              <div className="mt-8 space-y-3">
                <Link href={`/gigs/my-gigs/${gig.id}/edit`}>
                  <Button variant="outline" className="w-full rounded-lg border-gray-300 hover:bg-gray-100 hover:text-gray-800 transition-colors">
                    Editar Trabajo
                  </Button>
                </Link>
                <Button 
              ...
              <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Esta acción no se puede deshacer. Esto eliminará permanentemente el trabajo
                      y todas sus etapas y recompensas asociadas.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
          ...
          <Card className="border-none shadow-md bg-gradient-to-br from-blue-600 to-indigo-700 text-white">
            <CardContent className="p-6">
              ...
              <p className="text-blue-100 mb-4">¿Tienes preguntas sobre gestionar este trabajo o necesitas ayuda?</p>
              <Button variant="outline" className="w-full bg-white text-blue-700 hover:bg-blue-50 border-white">
                Contactar a Soporte
              </Button>
            </CardContent>
          </Card>
